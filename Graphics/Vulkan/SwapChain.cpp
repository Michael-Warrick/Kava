...
Graphics::SwapChain::SwapChain(GLFWwindow *window,
...
Graphics::SwapChain::~SwapChain() { cleanupSwapChain(); }
...
void Graphics::SwapChain::createSwapChain() {
    ...
    vk::SurfaceFormatKHR surfaceFormat =
        chooseSurfaceFormat(swapChainSupport.surfaceFormats);
    ...
    vk::SwapchainCreateInfoKHR createInfo =
        vk::SwapchainCreateInfoKHR()
            ...
            .setImageSharingMode(isConcurrentSharing
                                     ? vk::SharingMode::eConcurrent
                                     : vk::SharingMode::eExclusive)
            .setQueueFamilyIndexCount(isConcurrentSharing ? 2 : 0)
            .setPQueueFamilyIndices(isConcurrentSharing ? queueFamilyIndices
                                                        : nullptr)
    ...
    vk::Result result =
        m_LogicalDevice.createSwapchainKHR(&createInfo, nullptr, &m_SwapChain);
    if (result != vk::Result::eSuccess) {
        throw std::runtime_error("Failed to create swap chain! Error Code: " +
                                 vk::to_string(result));
    }
    ...
    if (result != vk::Result::eSuccess) {
        throw std::runtime_error(
            "Failed to get swap chain images! Error Code: " +
            vk::to_string(result));
    }
    ...
    if (result != vk::Result::eSuccess) {
        throw std::runtime_error(
            "Failed to get swap chain images with data! Error Code: " +
            vk::to_string(result));
    }
...
Graphics::SwapChain::QuerySwapChainSupport(vk::PhysicalDevice physicalDevice,
                                           vk::SurfaceKHR surface) {
    SwapChainSupportDetails details;
    vk::Result result = physicalDevice.getSurfaceCapabilitiesKHR(
        surface, &details.capabilities);
    if (result != vk::Result::eSuccess) {
        throw std::runtime_error(
            "Failed to get swap chain surface capabilities! Error Code: " +
            vk::to_string(result));
    }
    ...
    if (result != vk::Result::eSuccess) {
        throw std::runtime_error(
            "Failed to get swap chain surface formats! Error Code: " +
            vk::to_string(result));
    }
    ...
    if (formatCount != 0) {
        ...
        result = physicalDevice.getSurfaceFormatsKHR(
            surface, &formatCount, details.surfaceFormats.data());
        if (result != vk::Result::eSuccess) {
            throw std::runtime_error("Failed to get swap chain surface formats "
                                     "with data! Error Code: " +
                                     vk::to_string(result));
        }
    }
    ...
    if (result != vk::Result::eSuccess) {
        throw std::runtime_error(
            "Failed to get swap chain present modes! Error Code: " +
            vk::to_string(result));
    }
    ...
    if (presentModeCount != 0) {
        ...
        if (result != vk::Result::eSuccess) {
            throw std::runtime_error("Failed to get swap chain present modes "
                                     "with data! Error Code: " +
                                     vk::to_string(result));
        }
    }
...
vk::Extent2D Graphics::SwapChain::chooseExtent(
    ...
    if (capabilities.currentExtent.width !=
        ...
        actualExtent.setWidth(std::clamp(actualExtent.width,
                                         capabilities.minImageExtent.width,
                                         capabilities.maxImageExtent.width));
        actualExtent.setHeight(std::clamp(actualExtent.height,
...
void Graphics::SwapChain::createImageViews() {
    ...
    for (const auto &swapChainImage : m_SwapChainImages) {
        m_SwapChainImageViews.emplace_back(
            ImageView(m_LogicalDevice, swapChainImage, m_SwapChainImageFormat,
                      vk::ImageAspectFlagBits::eColor, 1)
                .get());
    }
}

void Graphics::SwapChain::createSwapChainFramebuffers(
    vk::RenderPass renderPass) {
    m_Framebuffer =
        Framebuffer(m_LogicalDevice, m_SwapChainImageViews, renderPass,
