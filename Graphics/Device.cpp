...
Graphics::Device::Device(const vk::Instance &instance,
...
Graphics::Device::~Device() { m_LogicalDevice.destroy(); }

vk::PhysicalDevice Graphics::Device::physical() const {
    return m_PhysicalDevice;
}

vk::Device Graphics::Device::logical() const { return m_LogicalDevice; }
...
void Graphics::Device::pickPhysicalDevice() {
    ...
    if (result != vk::Result::eSuccess) {
        throw std::runtime_error(
            "Failed to enumerate GPUs with selected data. Error code: " +
            vk::to_string(result));
    }
...
bool Graphics::Device::isPhysicalDeviceSuitable(
    vk::PhysicalDevice physicalDevice) {
    Queue::QueueFamilyIndices indices =
        Queue::FindQueueFamilies(physicalDevice, m_Surface);
    ...
    if (extensionsAreSupported) {
        SwapChain::SwapChainSupportDetails swapChainSupport =
            SwapChain::QuerySwapChainSupport(physicalDevice, m_Surface);
        swapChainIsAdequate = !swapChainSupport.surfaceFormats.empty() &&
                              !swapChainSupport.presentModes.empty();
    }

    vk::PhysicalDeviceFeatures supportedFeatures = physicalDevice.getFeatures();

    return indices.IsComplete() && extensionsAreSupported &&
           swapChainIsAdequate && supportedFeatures.samplerAnisotropy;
}

bool Graphics::Device::checkPhysicalDeviceExtensionSupport(
    vk::PhysicalDevice physicalDevice) {
    uint32_t extensionCount = 0;
    vk::Result result = physicalDevice.enumerateDeviceExtensionProperties(
        nullptr, &extensionCount, nullptr);
    if (result != vk::Result::eSuccess) {
        throw std::runtime_error(
            "Failed to check physical device extension support. Error code: " +
            vk::to_string(result));
    }
    ...
    if (result != vk::Result::eSuccess) {
        throw std::runtime_error("Failed to check physical device extension "
                                 "support with data. Error code: " +
                                 vk::to_string(result));
    }

    std::set<std::string> requiredExtensions(m_PhysicalDeviceExtensions.begin(),
                                             m_PhysicalDeviceExtensions.end());
...
void Graphics::Device::createLogicalDevice() {
        ...
        vk::DeviceCreateInfo()
            ...
            .setEnabledLayerCount(
                m_ValidationLayersEnabled
                    ...
                    : 0)
            .setPpEnabledLayerNames(m_ValidationLayersEnabled
                                        ? m_ValidationLayers.data()
                                        : nullptr);

    vk::Result result =
        m_PhysicalDevice.createDevice(&createInfo, nullptr, &m_LogicalDevice);
    if (result != vk::Result::eSuccess) {
        throw std::runtime_error(
    ...
    m_LogicalDevice.getQueue(indices.graphicsFamily.value(), 0,
                             &m_GraphicsQueue);
