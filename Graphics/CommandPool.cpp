...
Graphics::CommandPool::CommandPool(const vk::SurfaceKHR &surface,
                                   const vk::PhysicalDevice &physicalDevice,
                                   const vk::Device &logicalDevice)
    : m_Surface(surface), m_PhysicalDevice(physicalDevice),
      m_LogicalDevice(logicalDevice) {
    Queue::QueueFamilyIndices queueFamilyIndices =
    ...
    if (result != vk::Result::eSuccess) {
        throw std::runtime_error("Failed to create command pool! Error: " +
                                 vk::to_string(result));
    }
}
