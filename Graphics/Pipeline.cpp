...
Graphics::Pipeline::Pipeline(const vk::PhysicalDevice &physicalDevice,
    ...
    : m_PhysicalDevice(physicalDevice), m_LogicalDevice(logicalDevice),
      m_SwapChainImageFormat(format), m_MSAASamples(msaaSamples) {
    createRenderPass();
    createGraphicsPipeline("./Resources/Shaders/triangle.vert.spv",
...
void Graphics::Pipeline::createRenderPass() {
    ...
    vk::Result result = m_LogicalDevice.createRenderPass(
        &renderPassCreateInfo, nullptr, &m_RenderPass);
    if (result != vk::Result::eSuccess) {
        throw std::runtime_error("Failed to create render pass! Error: " +
                                 vk::to_string(result));
    }
}
...
void Graphics::Pipeline::createGraphicsPipeline(
    ...
    vk::Result result = m_LogicalDevice.createPipelineLayout(
        &pipelineLayoutInfo, nullptr, &m_PipelineLayout);
    if (result != vk::Result::eSuccess) {
        throw std::runtime_error("Failed to create pipeline layout! Error: " +
                                 vk::to_string(result));
    }
    ...
    result = m_LogicalDevice.createGraphicsPipelines(
        nullptr, 1, &graphicsPipelineInfo, nullptr, &m_GraphicsPipeline);
    if (result != vk::Result::eSuccess) {
        throw std::runtime_error("Failed to create graphics pipeline! Error: " +
                                 vk::to_string(result));
    }
...
Graphics::Pipeline::createShaderModule(const std::vector<char> &code) {
    ...
    vk::Result result =
        m_LogicalDevice.createShaderModule(&createInfo, nullptr, &shaderModule);
    if (result != vk::Result::eSuccess) {
