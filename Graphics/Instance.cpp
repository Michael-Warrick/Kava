...
vk::Instance Graphics::Instance::get() const { return m_Instance; }
...
bool Graphics::Instance::checkValidationLayerSupport() {
    ...
    if (result != vk::Result::eSuccess) {
        throw std::runtime_error(
            "Failed to enumerate instance layer count. Error code: " +
            vk::to_string(result));
    }

    std::vector<vk::LayerProperties> availableLayers(layerCount);

    result = vk::enumerateInstanceLayerProperties(&layerCount,
                                                  availableLayers.data());
    if (result != vk::Result::eSuccess) {
        throw std::runtime_error(
            "Failed to enumerate instance available layers. Error code: " +
            vk::to_string(result));
    }
...
std::vector<const char *> Graphics::Instance::getRequiredExtensions() const {
    ...
    if (m_IsRunningOnMacOS) {
        // Adding the portability extension (for MoltenVK driver compatibility
        // issue) + Setting flag
...
void Graphics::Instance::populateDebugMessengerCreateInfo(
...
VkResult Graphics::Instance::createDebugUtilsMessengerEXT(
...
void Graphics::Instance::destroyDebugUtilsMessengerEXT(
...
VKAPI_ATTR VkBool32 VKAPI_CALL Graphics::Instance::debugCallback(
